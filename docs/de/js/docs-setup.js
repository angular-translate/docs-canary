NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "keywords": "$sanitize $scope $translateprovider aber aktivieren aktivierte allerdings allgemeine allgemeinen angular angular-translate angularjs anwendungsbeispiele app applikation auch ausgabe ausgabeform ausgaben ausgegeben bei benutzung bereinigt bereits color config controller ctrl daf daher dann dar das dass dazu deaktiviert den der derzeit die dies dringlich dynamische dynamischer eigenen eine einen einer empfehlen en enable entfernt ergebnis ernsthaften es escape escaped escapen escapeparameters escapeparams escaping explizit feature folgende function gesamte gew gibt gilt grunds guide headline hello html inhalte innerhalb intern interpolation ist javascript kann konfiguriert korrekt lange love man mittels module myapp_escape myapp_escape_params myapp_not_escaped myapp_sanitize ng-controller ngsanitize nicht nichts noch null nur org overview owasp paragraph parametern pascalprecht php preferredlanguage regul sanitize sanitized sanitizeparameters security-strategy sein selbstverst sichere sicherheit sicherheitsl sorgt sp standard standardstrategien stehen strategie style synonym translate translate-values translations um und unicorn unsicherer usesanitizevaluestrategy var verf version verwendung vollst von warnhinweis welche werden werte wir wird wurde xss zeigen zu zur"
    },
    {
      "section": "guide",
      "id": "20_tools",
